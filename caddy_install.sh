...
Download_caddy(){
	...
	if [[ ${bit} == "x86_64" ]]; then
		wget --no-check-certificate -O "caddy_linux.tar.gz" "https://github.com/aptx17/boxtest/raw/main/caddy_v1.0.4_linux_amd64.tar.gz"
...
Service_caddy(){
	if [[ ${release} = "centos" ]]; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/aptx17/boxtest/main/caddy_debian -O /etc/init.d/caddy; then
			echo -e "${Error_font_prefix}[错误]${Font_suffix} Caddy服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/caddy
